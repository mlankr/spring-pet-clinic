...
public class DataLoader implements CommandLineRunner {
	...
	private void loadData() {
		...
		Owner owner1 = Owner.builder().firstname("Michael").lastname("Weston").address("King street 123").city("London")
				.telephone("0123456789").build();

		ownerService.save(owner1);

		Pet pet1 = Pet.builder().name("Shinobu").petType(savedDogType).owner(owner1)
				.birthDate(LocalDate.now().minusDays(330)).build();
		...
		Owner owner2 =
				Owner.builder().firstname("Fiona").lastname("Glenna").address("Queen street 123").city("Manchester")
						.telephone("0357903579").build();

		ownerService.save(owner2);

		Pet pet2 =
				Pet.builder().name("Coco").petType(savedCatType).owner(owner2).birthDate(LocalDate.now().minusDays(120))
						.build();
